...
class Utility:
    def __init__(self):
        pass
    def train_agent(self, render=False, verbose=False, num_episodes=1500,
        ...
        # number of episodes you want to train over
        ...
        for episode_count in range(num_episodes):
            # we have the ini
            state = env.reset()
            # uncomment if you want the car to do something random in the start to prevent overfitting
            ...
            while not env.is_done():
                ...
                current_state, action, reward, next_state, done, explored = agent.take_action(state)
                ...
                if not explored:
                    exploit_times += 1
                if episode_count % N == 0 and render:
                    env.render()
            ...
            # display stats
            episode_memory.append(Cumulative_reward)


            if verbose:
                print("Episode Count: ", episode_count,
                      "\t Cumulative Reward: ", Cumulative_reward,
                      "\t eps: ", agent.get_exp_rate())


        l = [m for m in episode_memory]
        epochs = list(range(len(l)))

        plt.clf
        plt.plot(epochs, l, 'b', label='f')
        env.close()
